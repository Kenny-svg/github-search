...
const Search = () => {
  ...
  const { isLoading, data } = useQuery(['repositories', searchTerm], async () => {
    ...
    <form className="flex items-center">   
      ...
      <ul className="grid grid-cols-2 md:grid-cols-5 gap-4 mt-10 border-t w-11/12 mx-auto">
              ...
              {data.map((result) => (
                <li className=" p-4 border rounded-lg text-center bg-emerald-400" key={result.id}>
                          <a target="_blank" href={result.html_url}>
              <div className=" p-4 border rounded-lg text-center bg-emerald-400" key={result.id}>
              <img className="rounded-full w-10 mx-auto" src={result.avatar_url} />
                <p>{result.login}</p>
